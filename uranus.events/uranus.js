...
function addPlanetModel(name, path, distance, scale = 1, rotSpeed = 0.2  , deg) {
  totalAssets += 1;
...
function init() {
  ...
  // Saturn Rings
  const capGeo = new THREE.SphereGeometry(7.5, 64, 32, 0, Math.PI * 2, 0, Math.PI / 4);
 
const capMat = new THREE.MeshStandardMaterial({ color: 0xffffff, emissive:0x00ffff , emissiveIntensity: 0.3,   opacity: 0.6 });
const capMesh = new THREE.Mesh(capGeo, capMat);
capMesh.position.set(20, 0, 0); // align with Uranus
scene.add(capMesh);
  ...
  composer.addPass(bloomPass);
   const container = document.querySelector('.Uranus-card');
...
rocket.addEventListener('click', () => {
  ...
  // Load Saturn model
  addPlanetModel("Uranus", "uranus(draco).glb", 20, 15 , 90);
}
