...
function createSunsetTexture() {
  ...
  gradient.addColorStop(0, '#1e3f66');   // deep twilight blue (top)
  gradient.addColorStop(0.4, '#C47DA0'); // pink mid-sky
  gradient.addColorStop(0.7, '#FF900E'); // orange near horizon
  gradient.addColorStop(1, '#FCDD2D');   // yellow glow at horizon
...
function init() {
  ...
  const elongationAngle = THREE.MathUtils.degToRad(47);
  const distanceFromSun = 500;
  // Venus (for later elongation simulation)
  const venusGeo = new THREE.SphereGeometry(27, 64, 64);
  const venusMat = new THREE.MeshStandardMaterial({
    color: 0xffffff,
    emissive: 0xadd86e,
    emissiveIntensity: 6,
  });
  const VenusX = sunX + Math.cos(elongationAngle)* distanceFromSun;
  const venusz = sunX + Math.sin(elongationAngle)* distanceFromSun;
  const venusMesh = new THREE.Mesh(venusGeo, venusMat);
  venusMesh.position.set(VenusX, 200, venusz);
  scene.add(venusMesh);
  ...
  const container = document.querySelector('.venus-info');
...
rocket.addEventListener('click', () => {
  ...
  addPlanetModel("roughy-surface", "Rocky-Surface(Draco).glb", 100, 20);
